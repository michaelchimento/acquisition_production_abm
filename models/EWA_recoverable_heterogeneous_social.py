...
def simulation(lock,master_sim_num,params_list):
    ...
    while True:
        ...
        if not flag and num_know_novel==N:
            #write_csv(file_path,sim_num,params_list,timestep,num_know_novel,beh_freqs,sum_cum_payoffs,agent_matrix_values)
...
if __name__=="__main__":
    ...
    new_directory_path="../concat_data/csv"
    ...
    df_concat.to_csv(join(new_directory_path,"EWA_inference_social.csv".format(s_param_values,memory_window_values[0],base_hazard)), index = False)
    ...
    #set final filename here
    df_concat.to_csv(join(new_directory_path,"EWA_inference_social_agents.csv"), index = False)
