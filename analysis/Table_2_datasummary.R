...
t4 = df %>%
  ...
  group_by(EWA_chi) %>%
  summarize(variable = "production conformity", meanTTD=mean(TTD), CI_TTD= paste0("[",round(PI(TTD)[1],2), ",", upperCI_TTD=round(PI(TTD)[2],2),"]"), meanTTFP=mean(TTFP), CI_TTFP= paste0("[",round(PI(TTFP)[1],2), ",", upperCI_TTD=round(PI(TTFP)[2],2),"]"), mean_perc_div = mean(divergence)*100, mean_d_order = mean(manhattan_divergence), CI_d_order= paste0("[",round(PI(manhattan_divergence)[1],2), ",", upperCI_TTD=round(PI(manhattan_divergence)[2],2),"]"), mean_d_time = mean(manhattan_delay), CI_d_time= paste0("[",round(PI(manhattan_delay)[1],2), ",", upperCI_TTD=round(PI(manhattan_delay)[2],2),"]"),) %>% rename(value= EWA_chi) %>% mutate(value=as.character(value))
...
t5 = df %>%
  ...
  summarize(variable = "memory window", meanTTD=mean(TTD), CI_TTD= paste0("[",round(PI(TTD)[1],2), ",", upperCI_TTD=round(PI(TTD)[2],2),"]"), meanTTFP=mean(TTFP), CI_TTFP= paste0("[",round(PI(TTFP)[1],2), ",", upperCI_TTD=round(PI(TTFP)[2],2),"]"), mean_perc_div = mean(divergence)*100, mean_d_order = mean(manhattan_divergence), CI_d_order= paste0("[",round(PI(manhattan_divergence)[1],2), ",", upperCI_TTD=round(PI(manhattan_divergence)[2],2),"]"), mean_d_time = mean(manhattan_delay), CI_d_time= paste0("[",round(PI(manhattan_delay)[1],2), ",", upperCI_TTD=round(PI(manhattan_delay)[2],2),"]"),) %>% rename(value= memory_window) %>% mutate(value=as.character(value))

#### 3.2 Phi ####
t6 = df %>%
  filter(graph_type=="random regular", memory_window==10, EWA_chi=="linear bias", EWA_sigma=="medium", EWA_alpha=="risk-neutral") %>%
  group_by(EWA_rho) %>%
  summarize(variable = "recent experience bias", meanTTD=mean(TTD), CI_TTD= paste0("[",round(PI(TTD)[1],2), ",", upperCI_TTD=round(PI(TTD)[2],2),"]"), meanTTFP=mean(TTFP), CI_TTFP= paste0("[",round(PI(TTFP)[1],2), ",", upperCI_TTD=round(PI(TTFP)[2],2),"]"), mean_perc_div = mean(divergence)*100, mean_d_order = mean(manhattan_divergence), CI_d_order= paste0("[",round(PI(manhattan_divergence)[1],2), ",", upperCI_TTD=round(PI(manhattan_divergence)[2],2),"]"), mean_d_time = mean(manhattan_delay), CI_d_time= paste0("[",round(PI(manhattan_delay)[1],2), ",", upperCI_TTD=round(PI(manhattan_delay)[2],2),"]"),) %>% rename(value= EWA_rho)%>% mutate(value=as.character(value))

#### 3.2 Tau ####
t7 = df %>%
  filter(graph_type=="random regular", memory_window==10, EWA_chi=="linear bias", EWA_sigma=="medium", EWA_rho=="medium") %>%
  group_by(EWA_alpha) %>%
  summarize(variable = "risk-appetite", meanTTD=mean(TTD), CI_TTD= paste0("[",round(PI(TTD)[1],2), ",", upperCI_TTD=round(PI(TTD)[2],2),"]"), meanTTFP=mean(TTFP), CI_TTFP= paste0("[",round(PI(TTFP)[1],2), ",", upperCI_TTD=round(PI(TTFP)[2],2),"]"), mean_perc_div = mean(divergence)*100, mean_d_order = mean(manhattan_divergence), CI_d_order= paste0("[",round(PI(manhattan_divergence)[1],2), ",", upperCI_TTD=round(PI(manhattan_divergence)[2],2),"]"), mean_d_time = mean(manhattan_delay), CI_d_time= paste0("[",round(PI(manhattan_delay)[1],2), ",", upperCI_TTD=round(PI(manhattan_delay)[2],2),"]"),) %>% rename(value= EWA_alpha) %>% mutate(value=as.character(value))
t7
