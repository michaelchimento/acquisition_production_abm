...
df = df_equiv_payoffs_acq_prod %>% filter(NBDA_s_param==10) %>% group_by(sim, graph_type, NBDA_s_param, EWA_soc_info_weight, EWA_recent_payoff_weight,conformity) %>% summarize(timestep=max(timestep_acquisition_b))
...
df_delta = df_equiv_payoffs_acq_prod %>% filter(conformity=="None", NBDA_s_param==10) %>% mutate(delta=timestep_production_b - timestep_acquisition_b)
