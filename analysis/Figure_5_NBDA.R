...
df_div = df_div %>% select(sim,delta,order_acquisition,order_production) %>% group_by(sim) %>% summarize(avgdelta=mean(delta), divergence = 1-sum(order_acquisition==order_production)/n())
