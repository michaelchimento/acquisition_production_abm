...
load("../model_outputs/Rda_files/df_social_inference_preprogrammed.Rda")
load("../model_outputs/Rda_files/df_asocial_inference_preprogrammed.Rda")

df_diffusion$type="a"
df_asocial_inference$type="b"

library(kableExtra)
df = rbind(df_diffusion,df_asocial_inference)
...
t = bind_rows(t1,t2)
t = t %>% pivot_wider(names_from = type, values_from = c(mean,HPDI_89)) %>% relocate(variable) %>% select(variable,value,mean_a, HPDI_89_a, mean_b, HPDI_89_b)
...
kable(t[2:ncol(t)],booktabs = TRUE,
  ...
  add_header_above(c(" "=1,"Heterogenous:\nsocial learning"=2,"Heterogenous:\nasocial learning"=2))
